    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for epoch in log_dict.keys():
            if args.include_outliers:
                all_times.append(np.array(log_dict[epoch]["data_time"]))
            else:
                all_times.append(np.array(log_dict[epoch]["data_time"][1:]))
...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if metric not in log_dict[epochs[0]] and metric != "loss_direction":
                raise KeyError(
            ...
            if "mAP" in metric:
                ...
                for epoch in epochs:
                    ...
                    if metric == "loss_direction":
                        metric_ = 'loc_loss_elem'
                        # print(y[0,:,-1])
                        ys.append(np.array(log_dict[epoch][metric_])[: len(iters),:,-1])
                    else:
                        ys.append(np.array(log_dict[epoch][metric][: len(iters)]))
                        print(np.array(log_dict[epoch][metric][-1]))
